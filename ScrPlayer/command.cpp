...
get_adb_command(void) {
    if (!adb_command) {
        //size_t len;
        //errno_t err = _dupenv_s(adb_command, &len, "ADB");
        //if (err) exit(-1);
        //if (!adb_command)
            adb_command = "adb";
    }
...
adb_execute(const char *serial, const char *const adb_cmd[], size_t len) {
    const char **cmd = (const char**)calloc(sizeof(char*),len + 4);
    int i;
    process_t process;
...
adb_forward(const char *serial, uint16_t local_port,
    ...
    sprintf_s(local,10, "tcp:%" PRIu16, local_port);
